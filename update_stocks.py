...
def generate_news_summary(stock_code: str, stock_name: str) -> Optional[str]:
    ...
    news_list = fetch_stock_news(stock_code, limit=5)
    if not news_list:
    ...
    if not matched:
        # fallback: 첫 헤드라인에서 핵심 명사 추출 시도
        first_title = news_list[0].get("title", "")
        # 간단한 키워드 추출: 주요 명사 패턴
        fallback_keywords = []
        noun_patterns = ["반도체", "전자", "금융", "IT", "바이오", "제약", "자동차", "화학", "철강", "에너지", "통신", "미디어", "유통", "건설", "기계", "소재", "부품"]
        for noun in noun_patterns:
            if noun in first_title:
                fallback_keywords.append(noun)
        if fallback_keywords:
            summary = f"{', '.join(fallback_keywords[:2])} 관련 최신 동향"
        else:
            summary = "실시간 뉴스 분석 중"
    elif len(matched) == 1:
        summary = matched[0]
        ...
        summary = f"{matched[0]}, {matched[1]} 등 {len(matched)}가지 긍정적 동향"
